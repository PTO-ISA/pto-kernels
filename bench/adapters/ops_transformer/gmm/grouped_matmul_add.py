...
from pto_kernels.ops.gmm.grouped_matmul_add.runtime import (
    ...
    run_torch_npu_grouped_matmul_add,
)


def describe(repo_root, spec):
    summary = describe_baseline(repo_root, "gmm", "grouped_matmul_add", spec.inventory_ref)
    summary["runtime_entrypoint"] = "torch_npu.npu_grouped_matmul_add"
...
def compile_kernel(repo_root, spec, artifacts_dir):
    if not hasattr(torch_npu, "npu_grouped_matmul_add"):
        return {
            "status": "blocked",
            "reason": "torch_npu does not expose npu_grouped_matmul_add on this environment.",
        }
    return {
        "status": "runtime_builtin",
        "entrypoint": "torch_npu.npu_grouped_matmul_add",
    }
    ...
    try:
        ...
        for variant in VARIANTS:
            ...
            for _ in range(spec.bench.warmup):
                run_torch_npu_grouped_matmul_add(inputs)
            ...
            for _ in range(spec.bench.repeat):
                ...
                output = run_torch_npu_grouped_matmul_add(inputs)
                torch.npu.synchronize()
        ...
        report_path = Path(artifacts_dir) / "ops_transformer_grouped_matmul_add_benchmark.json"
