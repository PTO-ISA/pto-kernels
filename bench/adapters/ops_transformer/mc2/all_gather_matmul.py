...
def describe(repo_root, spec):
    summary = describe_baseline(
        repo_root, "mc2", "all_gather_matmul", spec.inventory_ref
    )
...
def benchmark(repo_root, spec, artifacts_dir):
            ...
            report = {
                ...
                "correctness": {
                    "output_max_abs_diff": max(
                        float(item["correctness"]["output_max_abs_diff"])
                        for item in variant_reports
                    ),
                    "gather_max_abs_diff": max(
                        float(item["correctness"]["gather_max_abs_diff"])
                        for item in variant_reports
                    ),
    ...
    report_path = (
        Path(artifacts_dir) / "ops_transformer_all_gather_matmul_benchmark.json"
    )
