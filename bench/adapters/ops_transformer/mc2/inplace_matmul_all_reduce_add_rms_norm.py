...
from pto_kernels.ops.mc2.inplace_matmul_all_reduce_add_rms_norm.runtime import (
    VARIANT,
    VARIANTS,
    baseline_blocker,
...
def describe(repo_root, spec):
    ...
    summary["runtime_entrypoint"] = (
        "torch_npu.npu_mm_all_reduce_base + torch_npu.npu_add_rms_norm"
    )
    ...
    summary["baseline_note"] = (
        "The constrained inplace seed uses the installed split runtime contract "
        "npu_mm_all_reduce_base + npu_add_rms_norm and validates the residual alias contract in the harness."
    )
    return summary
...
def benchmark(repo_root, spec, artifacts_dir):
    ...
    try:
        ...
        report["reason"] = (
            f"Distributed inplace_matmul_all_reduce_add_rms_norm baseline failed: {exc}"
        )
    else:
        if any(item.get("status") != "ok" for item in variant_reports):
            ...
            norm_diff = max(
                float(item["correctness"]["norm_max_abs_diff"])
                for item in variant_reports
            )
            max_abs_diff = max(
    ...
    report_path = (
        Path(artifacts_dir)
        / "ops_transformer_inplace_matmul_all_reduce_add_rms_norm_benchmark.json"
    )
