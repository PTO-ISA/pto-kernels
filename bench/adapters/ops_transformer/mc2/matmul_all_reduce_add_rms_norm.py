...
def describe(repo_root, spec):
    summary = describe_baseline(
        repo_root, "mc2", "matmul_all_reduce_add_rms_norm", spec.inventory_ref
    )
...
def benchmark(repo_root, spec, artifacts_dir):
    ...
    try:
        ...
        report["reason"] = (
            f"Distributed matmul_all_reduce_add_rms_norm baseline failed: {exc}"
        )
    else:
        if any(item.get("status") != "ok" for item in variant_reports):
            ...
            report = {
                ...
                "reason": first_blocked.get(
                    "reason",
                    "Distributed matmul_all_reduce_add_rms_norm baseline failed.",
                ),
                "variant_reports": variant_reports,
            }
        else:
            y_diff = max(
                float(item["correctness"]["y_max_abs_diff"]) for item in variant_reports
            )
            norm_diff = max(
    ...
    report_path = (
        Path(artifacts_dir)
        / "ops_transformer_matmul_all_reduce_add_rms_norm_benchmark.json"
    )
