...
from pto_kernels.ops.moe.moe_token_unpermute_with_routing_map_grad.runtime import (
    ...
    run_torch_npu_moe_token_unpermute_with_routing_map_grad,
)
...
def describe(repo_root, spec):
    ...
    summary["runtime_entrypoint"] = "torch_npu.npu_moe_token_unpermute_with_routing_map_grad"
...
def compile_kernel(repo_root, spec, artifacts_dir):
    ...
    if not hasattr(torch_npu, "npu_moe_token_unpermute_with_routing_map_grad"):
        return {
            "status": "blocked",
            "reason": "torch_npu does not expose npu_moe_token_unpermute_with_routing_map_grad on this environment.",
        }
    return {
        "status": "runtime_builtin",
        "entrypoint": "torch_npu.npu_moe_token_unpermute_with_routing_map_grad",
        "note": (
            "Baseline execution uses the installed custom ops runtime package. "
            "The constrained slice fixes probsOptional=None, topK=1, dropAndPad=false, and "
    ...
    try:
        ...
        for variant in VARIANTS:
            ...
            for _ in range(spec.bench.warmup):
                run_torch_npu_moe_token_unpermute_with_routing_map_grad(inputs)
            ...
            for _ in range(spec.bench.repeat):
                ...
                output = run_torch_npu_moe_token_unpermute_with_routing_map_grad(inputs)
                torch.npu.synchronize()
        ...
        report_path = Path(artifacts_dir) / "ops_transformer_moe_token_unpermute_with_routing_map_grad_benchmark.json"
