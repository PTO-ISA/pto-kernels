...
from pto_kernels.ops.moe.moe_gating_top_k_softmax.runtime import (
    ...
    run_torch_npu_moe_gating_top_k_softmax,
)


def describe(repo_root, spec):
    summary = describe_baseline(repo_root, "moe", "moe_gating_top_k_softmax", spec.inventory_ref)
    summary["runtime_entrypoint"] = "torch_npu.npu_moe_gating_top_k_softmax"
...
def compile_kernel(repo_root, spec, artifacts_dir):
    ...
    if not hasattr(torch_npu, "npu_moe_gating_top_k_softmax"):
        return {
            "status": "blocked",
            "reason": "torch_npu does not expose npu_moe_gating_top_k_softmax on this environment.",
        }
    return {
        "status": "runtime_builtin",
        "entrypoint": "torch_npu.npu_moe_gating_top_k_softmax",
        "note": (
            "Baseline execution uses the installed custom ops runtime package. "
            "The constrained slice fixes topK=1, finishedOptional=None, and 2D input."
        ),
    }
    ...
    try:
        ...
        for variant in VARIANTS:
            ...
            for _ in range(spec.bench.warmup):
                run_torch_npu_moe_gating_top_k_softmax(inputs)
            ...
            for _ in range(spec.bench.repeat):
                ...
                output = run_torch_npu_moe_gating_top_k_softmax(inputs)
                torch.npu.synchronize()
            ...
            variant_reports.append(
                {
                    ...
                    "correctness": {
                        ...
                        "passes": bool(y_diff <= spec.correctness.atol and expert_diff == 0 and row_diff == 0),
                    },
                }
            )
        ...
        report_path = Path(artifacts_dir) / "ops_transformer_moe_gating_top_k_softmax_benchmark.json"
