...
from pto_kernels.ops.moe.moe_token_unpermute.runtime import (
    ...
    run_torch_npu_moe_token_unpermute,
)


def describe(repo_root, spec):
    summary = describe_baseline(repo_root, "moe", "moe_token_unpermute", spec.inventory_ref)
    summary["runtime_entrypoint"] = "torch_npu.npu_moe_token_unpermute"
...
def compile_kernel(repo_root, spec, artifacts_dir):
    ...
    if not hasattr(torch_npu, "npu_moe_token_unpermute"):
        return {
            "status": "blocked",
            "reason": "torch_npu does not expose npu_moe_token_unpermute on this environment.",
        }
    return {
        "status": "runtime_builtin",
        "entrypoint": "torch_npu.npu_moe_token_unpermute",
        "note": (
...
def benchmark(repo_root, spec, artifacts_dir):
    ...
    try:
        ...
        for variant in VARIANTS:
            ...
            for _ in range(spec.bench.warmup):
                run_torch_npu_moe_token_unpermute(inputs)
            ...
            for _ in range(spec.bench.repeat):
                ...
                output = run_torch_npu_moe_token_unpermute(inputs)
                torch.npu.synchronize()
        ...
        report_path = Path(artifacts_dir) / "ops_transformer_moe_token_unpermute_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "top1_inverse_permute_no_probs",
    }
    report_path = Path(artifacts_dir) / "ops_transformer_moe_token_unpermute_benchmark.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    report["report_path"] = str(report_path)
