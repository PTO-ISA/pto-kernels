...
from pto_kernels.ops.moe.moe_finalize_routing.runtime import (
    ...
    run_torch_npu_moe_finalize_routing,
)


def describe(repo_root, spec):
    summary = describe_baseline(repo_root, "moe", "moe_finalize_routing", spec.inventory_ref)
    summary["runtime_entrypoint"] = "torch_npu.npu_moe_finalize_routing"
...
def compile_kernel(repo_root, spec, artifacts_dir):
    ...
    if not hasattr(torch_npu, "npu_moe_finalize_routing"):
        return {
            "status": "blocked",
            "reason": "torch_npu does not expose npu_moe_finalize_routing on this environment.",
        }
    return {
        "status": "runtime_builtin",
        "entrypoint": "torch_npu.npu_moe_finalize_routing",
        "note": (
            "Baseline execution uses the installed custom ops runtime package. "
            "The constrained slice fixes topK=1, x2Optional=None, and dropPadMode=0."
        ),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    ...
    try:
        ...
        for variant in VARIANTS:
            ...
            for _ in range(spec.bench.warmup):
                run_torch_npu_moe_finalize_routing(inputs)
            ...
            for _ in range(spec.bench.repeat):
                ...
                output = run_torch_npu_moe_finalize_routing(inputs)
                torch.npu.synchronize()
        ...
        report_path = Path(artifacts_dir) / "ops_transformer_moe_finalize_routing_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "top1_finalize_routing_no_skip2",
    }
    report_path = Path(artifacts_dir) / "ops_transformer_moe_finalize_routing_benchmark.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    report["report_path"] = str(report_path)
