...
from pto_kernels.ops.moe.moe_compute_expert_tokens.runtime import (
    VARIANT,
    VARIANTS,
    make_compute_expert_tokens_inputs,
    run_torch_npu_moe_compute_expert_tokens,
)


def describe(repo_root, spec):
    summary = describe_baseline(repo_root, "moe", "moe_compute_expert_tokens", spec.inventory_ref)
    summary["runtime_entrypoint"] = "torch.ops.npu.npu_moe_compute_expert_tokens"
...
def compile_kernel(repo_root, spec, artifacts_dir):
    ...
    if not hasattr(torch.ops.npu, "npu_moe_compute_expert_tokens"):
        return {
            "status": "blocked",
            "reason": "torch.ops.npu does not expose npu_moe_compute_expert_tokens on this environment.",
        }
    return {
        "status": "runtime_builtin",
        "entrypoint": "torch.ops.npu.npu_moe_compute_expert_tokens",
        "note": "Baseline execution uses the installed custom ops runtime package.",
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    ...
    try:
        ...
        for variant in VARIANTS:
            inputs = make_compute_expert_tokens_inputs(variant, device_index=int(spec.device.get("id", 0)))
            for _ in range(spec.bench.warmup):
                run_torch_npu_moe_compute_expert_tokens(inputs)
            ...
            for _ in range(spec.bench.repeat):
                ...
                output = run_torch_npu_moe_compute_expert_tokens(inputs)
                torch.npu.synchronize()
            ...
            out_diff = (output.cpu() - inputs["reference_out"]).abs().max().item()
        ...
        report_path = Path(artifacts_dir) / "ops_transformer_moe_compute_expert_tokens_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "upper_bound_positions_per_expert",
    }
    report_path = Path(artifacts_dir) / "ops_transformer_moe_compute_expert_tokens_benchmark.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    report["report_path"] = str(report_path)
