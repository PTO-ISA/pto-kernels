...
from pto_kernels.ops.posembedding.rotary_position_embedding.runtime import (
    VARIANTS,
    make_inputs,
    run_torch_npu_rotary_position_embedding,
)


def describe(repo_root, spec):
    summary = describe_baseline(repo_root, "posembedding", "rotary_position_embedding", spec.inventory_ref)
    summary["runtime_entrypoint"] = "torch_npu.npu_rotary_mul"
...
def compile_kernel(repo_root, spec, artifacts_dir):
    if not hasattr(torch_npu, "npu_rotary_mul"):
        return {
            "status": "blocked",
            "reason": "torch_npu does not expose npu_rotary_mul on this environment.",
        }
    return {
        "status": "runtime_builtin",
        "entrypoint": "torch_npu.npu_rotary_mul",
        "note": "Baseline execution uses the installed half-mode rotary runtime path exposed through torch_npu.npu_rotary_mul.",
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    ...
    try:
        for variant in VARIANTS:
            ...
            for _ in range(spec.bench.warmup):
                run_torch_npu_rotary_position_embedding(inputs)
            ...
            for _ in range(spec.bench.repeat):
                ...
                output = run_torch_npu_rotary_position_embedding(inputs)
                torch.npu.synchronize()
            ...
            max_abs_diff = (output.float().cpu() - inputs["reference"]).abs().max().item()
        ...
        report_path = Path(artifacts_dir) / "ops_transformer_rotary_position_embedding_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "fp16_half_rotary_bsnd_bnsd_d128",
    }
    report_path = Path(artifacts_dir) / "ops_transformer_rotary_position_embedding_benchmark.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    report["report_path"] = str(report_path)
