...
from pto_kernels.ops.posembedding.rope_quant_kvcache.runtime import (
    VARIANTS,
    make_inputs,
    run_torch_npu_rope_quant_kvcache,
)


def describe(repo_root, spec):
    summary = describe_baseline(repo_root, "posembedding", "rope_quant_kvcache", spec.inventory_ref)
    summary["runtime_entrypoint"] = "torch_npu.npu_rope_quant_kvcache"
    summary["seed_variant"] = {"variants": [variant.as_dict() for variant in VARIANTS]}
    return summary


def compile_baseline(repo_root, spec):
    if not hasattr(torch_npu, "npu_rope_quant_kvcache"):
        return {"status": "blocked", "reason": "torch_npu does not expose npu_rope_quant_kvcache on this environment."}
    return {
        "status": "runtime_builtin",
        "entrypoint": "torch_npu.npu_rope_quant_kvcache",
        "note": "Baseline execution relies on the installed runtime builtin for the constrained fp16 rope_quant_kvcache seed.",
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            ...
            for _ in range(spec.bench.warmup):
                run_torch_npu_rope_quant_kvcache(inputs)
            ...
            for _ in range(spec.bench.repeat):
                ...
                output = run_torch_npu_rope_quant_kvcache(inputs)
                torch.npu.synchronize()
        ...
        report = {"status": "blocked", "reason": f"Baseline execution failed: {exc}", "entrypoint": "torch_npu.npu_rope_quant_kvcache"}
        report_path = Path(artifacts_dir) / "ops_transformer_rope_quant_kvcache_benchmark.json"
        report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
        report["report_path"] = str(report_path)
        return report

    max_abs_diff = max(
        max(item["correctness"].values())
    ...
    report = {
        ...
        "reference_contract": "fp16_half_rope_quant_kvcache_unit_scale_bsnd_2d",
    }
    report_path = Path(artifacts_dir) / "ops_transformer_rope_quant_kvcache_benchmark.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    report["report_path"] = str(report_path)
