...
from pto_kernels.ops.posembedding.rope_with_sin_cos_cache.runtime import VARIANTS, make_inputs, run_pto_variant


KERNEL = "python/pto_kernels/ops/posembedding/rope_with_sin_cos_cache/kernel.py"
META = "python/pto_kernels/ops/posembedding/rope_with_sin_cos_cache/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_ROPE_WITH_SIN_COS_CACHE_TOTAL_ROWS": str(variant.batch),
        "PTO_ROPE_WITH_SIN_COS_CACHE_BLOCK_DIM": os.environ.get("PTO_ROPE_WITH_SIN_COS_CACHE_BLOCK_DIM", "4"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            ...
            spec_obj = importlib.util.spec_from_file_location(
                f"pto_rope_with_sin_cos_cache_kernel_{variant.label}",
                kernel_file,
            )
            ...
            with temporary_env(_variant_env(variant)):
                ...
                correctness = {
                    "query_max_abs_diff": (query_out.float().cpu() - reference_query).abs().max().item(),
                    "key_max_abs_diff": (key_out.float().cpu() - reference_key).abs().max().item(),
                }
        ...
        report_path = Path(artifacts_dir) / "ptodsl_rope_with_sin_cos_cache_benchmark.json"
        report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
        report["report_path"] = str(report_path)
        return report

    max_abs_diff = max(
        max(item["correctness"]["query_max_abs_diff"], item["correctness"]["key_max_abs_diff"])
    ...
    report = {
        ...
        "reference_contract": "fp16_half_rope_with_sin_cos_cache_nd_d64",
    }
    report_path = Path(artifacts_dir) / "ptodsl_rope_with_sin_cos_cache_benchmark.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    report["report_path"] = str(report_path)
