...
from pto_kernels.ops.posembedding.interleave_rope.runtime import VARIANTS, make_inputs, run_pto_variant


KERNEL = "python/pto_kernels/ops/posembedding/interleave_rope/kernel.py"
META = "python/pto_kernels/ops/posembedding/interleave_rope/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_INTERLEAVE_ROPE_TOTAL_ROWS": str(variant.total_rows),
        "PTO_INTERLEAVE_ROPE_BLOCK_DIM": os.environ.get("PTO_INTERLEAVE_ROPE_BLOCK_DIM", "4"),
    }
    ...
    try:
        ...
        for variant in VARIANTS:
            ...
            spec_obj = importlib.util.spec_from_file_location(
                f"pto_interleave_rope_kernel_{variant.label}",
                kernel_file,
            )
        ...
        report_path = Path(artifacts_dir) / "ptodsl_interleave_rope_benchmark.json"
