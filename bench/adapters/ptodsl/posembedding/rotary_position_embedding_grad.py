...
KERNEL = "python/pto_kernels/ops/posembedding/rotary_position_embedding_grad/kernel.py"
META = "python/pto_kernels/ops/posembedding/rotary_position_embedding_grad/meta.py"
...
def _variant_env() -> dict[str, str]:
    return {
        "PTO_ROTARY_POSITION_EMBEDDING_GRAD_TOTAL_ROWS": "64",
        "PTO_ROTARY_POSITION_EMBEDDING_GRAD_BLOCK_DIM": os.environ.get(
            "PTO_ROTARY_POSITION_EMBEDDING_GRAD_BLOCK_DIM",
            "4",
        ),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            ...
            spec_obj = importlib.util.spec_from_file_location(
                f"pto_rotary_position_embedding_grad_kernel_{variant.label}",
                kernel_file,
            )
        ...
        report_path = Path(artifacts_dir) / "ptodsl_rotary_position_embedding_grad_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "fp16_half_rotary_grad_bsnd_bnsd_d128_no_reduce",
    }
    report_path = Path(artifacts_dir) / "ptodsl_rotary_position_embedding_grad_benchmark.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    report["report_path"] = str(report_path)
