...
KERNEL = "python/pto_kernels/ops/posembedding/rotary_position_embedding/kernel.py"
META = "python/pto_kernels/ops/posembedding/rotary_position_embedding/meta.py"
...
def _variant_env() -> dict[str, str]:
    return {
        "PTO_ROTARY_POSITION_EMBEDDING_TOTAL_ROWS": "64",
        "PTO_ROTARY_POSITION_EMBEDDING_BLOCK_DIM": os.environ.get("PTO_ROTARY_POSITION_EMBEDDING_BLOCK_DIM", "4"),
    }
    ...
    try:
        ...
        for variant in VARIANTS:
            ...
            spec_obj = importlib.util.spec_from_file_location(
                f"pto_rotary_position_embedding_kernel_{variant.label}",
                kernel_file,
            )
            if spec_obj is None or spec_obj.loader is None:
                raise ImportError(f"Unable to import {kernel_file}")

            with temporary_env(_variant_env()):
        ...
        report_path = Path(artifacts_dir) / "ptodsl_rotary_position_embedding_benchmark.json"
