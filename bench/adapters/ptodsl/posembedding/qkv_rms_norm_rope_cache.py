...
from pto_kernels.ops.posembedding.qkv_rms_norm_rope_cache.runtime import VARIANTS, make_inputs, run_pto_variant


KERNEL = "python/pto_kernels/ops/posembedding/qkv_rms_norm_rope_cache/kernel.py"
META = "python/pto_kernels/ops/posembedding/qkv_rms_norm_rope_cache/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_QKV_RMS_ROPE_CACHE_TOTAL_ROWS": str(variant.batch),
        "PTO_QKV_RMS_ROPE_CACHE_BLOCK_DIM": os.environ.get("PTO_QKV_RMS_ROPE_CACHE_BLOCK_DIM", "4"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            ...
            spec_obj = importlib.util.spec_from_file_location(f"pto_qkv_rms_norm_rope_cache_kernel_{variant.label}", kernel_file)
        ...
        report_path = Path(artifacts_dir) / "ptodsl_qkv_rms_norm_rope_cache_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "fp16_qkv_rms_norm_half_rope_quant_kvcache_bsnd_2d",
    }
    report_path = Path(artifacts_dir) / "ptodsl_qkv_rms_norm_rope_cache_benchmark.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    report["report_path"] = str(report_path)
