...
from pto_kernels.ops.attention.fused_infer_attention_score.runtime import (
    ...
    run_pto_fused_infer_attention_score_variant,
)
...
def _variant_env(variant) -> dict[str, str]:
    ...
    return {
        "PTO_FUSED_INFER_ATTENTION_SCORE_Q_SEQ": str(variant.q_seq),
        "PTO_FUSED_INFER_ATTENTION_SCORE_KV_SEQ": str(variant.kv_seq),
        "PTO_FUSED_INFER_ATTENTION_SCORE_HEAD_DIM": str(variant.head_dim),
        "PTO_FUSED_INFER_ATTENTION_SCORE_QK_BASE_M": os.environ.get("PTO_FUSED_INFER_ATTENTION_SCORE_QK_BASE_M", "16"),
        "PTO_FUSED_INFER_ATTENTION_SCORE_QK_BASE_N": os.environ.get("PTO_FUSED_INFER_ATTENTION_SCORE_QK_BASE_N", "16"),
        "PTO_FUSED_INFER_ATTENTION_SCORE_QK_BASE_K": os.environ.get("PTO_FUSED_INFER_ATTENTION_SCORE_QK_BASE_K", str(min(variant.head_dim, 64))),
        "PTO_FUSED_INFER_ATTENTION_SCORE_QK_BLOCK_DIM": os.environ.get("PTO_FUSED_INFER_ATTENTION_SCORE_QK_BLOCK_DIM", "8"),
        "PTO_FUSED_INFER_ATTENTION_SCORE_PV_BASE_M": os.environ.get("PTO_FUSED_INFER_ATTENTION_SCORE_PV_BASE_M", "16"),
        "PTO_FUSED_INFER_ATTENTION_SCORE_PV_BASE_N": os.environ.get("PTO_FUSED_INFER_ATTENTION_SCORE_PV_BASE_N", str(min(variant.head_dim, 64))),
        "PTO_FUSED_INFER_ATTENTION_SCORE_PV_BASE_K": os.environ.get("PTO_FUSED_INFER_ATTENTION_SCORE_PV_BASE_K", "16"),
        "PTO_FUSED_INFER_ATTENTION_SCORE_PV_BLOCK_DIM": os.environ.get("PTO_FUSED_INFER_ATTENTION_SCORE_PV_BLOCK_DIM", "8"),
        "PTO_FUSED_INFER_ATTENTION_SCORE_SOFTMAX_BLOCK_DIM": softmax_block_dim,
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                for _ in range(spec.bench.warmup):
                    run_pto_fused_infer_attention_score_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_fused_infer_attention_score_variant(wrapper, inputs)
                    torch.npu.synchronize()
        ...
        report_path = Path(artifacts_dir) / "ptodsl_fused_infer_attention_score_benchmark.json"
