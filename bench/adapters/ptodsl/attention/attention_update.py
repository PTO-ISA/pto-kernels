...
from pto_kernels.ops.attention.attention_update.runtime import (
    ...
    run_pto_attention_update_variant,
)


KERNEL = "python/pto_kernels/ops/attention/attention_update/kernel.py"
META = "python/pto_kernels/ops/attention/attention_update/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_ATTENTION_UPDATE_ROWS": str(variant.rows),
        "PTO_ATTENTION_UPDATE_HEAD_DIM": str(variant.head_dim),
        "PTO_ATTENTION_UPDATE_BLOCK_DIM": os.environ.get("PTO_ATTENTION_UPDATE_BLOCK_DIM", "8"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                for _ in range(spec.bench.warmup):
                    run_pto_attention_update_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_attention_update_variant(wrapper, inputs)
                    torch.npu.synchronize()
        ...
        report_path = Path(artifacts_dir) / "ptodsl_attention_update_benchmark.json"
    ...
    report = {
        ...
        "correctness": {
            ...
            "passes": bool(all(item["correctness"]["out_max_abs_diff"] <= spec.correctness.atol for item in variant_reports)),
        },
