...
from pto_kernels.ops.attention.incre_flash_attention.runtime import (
    ...
    run_pto_incre_flash_attention_variant,
)


KERNEL = "python/pto_kernels/ops/attention/incre_flash_attention/kernel.py"
META = "python/pto_kernels/ops/attention/incre_flash_attention/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    softmax_block_dim = "1" if variant.kv_seq <= 16 else os.environ.get(
        "PTO_INCRE_FLASH_ATTENTION_SOFTMAX_BLOCK_DIM", "8"
    )
    return {
        "PTO_INCRE_FLASH_ATTENTION_Q_HEADS": str(variant.q_heads),
        "PTO_INCRE_FLASH_ATTENTION_KV_SEQ": str(variant.kv_seq),
        "PTO_INCRE_FLASH_ATTENTION_HEAD_DIM": str(variant.head_dim),
        "PTO_INCRE_FLASH_ATTENTION_QK_BASE_M": os.environ.get("PTO_INCRE_FLASH_ATTENTION_QK_BASE_M", "16"),
        "PTO_INCRE_FLASH_ATTENTION_QK_BASE_N": os.environ.get("PTO_INCRE_FLASH_ATTENTION_QK_BASE_N", "16"),
        "PTO_INCRE_FLASH_ATTENTION_QK_BASE_K": os.environ.get("PTO_INCRE_FLASH_ATTENTION_QK_BASE_K", str(min(variant.head_dim, 64))),
        "PTO_INCRE_FLASH_ATTENTION_QK_BLOCK_DIM": os.environ.get("PTO_INCRE_FLASH_ATTENTION_QK_BLOCK_DIM", "8"),
        "PTO_INCRE_FLASH_ATTENTION_PV_BASE_M": os.environ.get("PTO_INCRE_FLASH_ATTENTION_PV_BASE_M", "16"),
        "PTO_INCRE_FLASH_ATTENTION_PV_BASE_N": os.environ.get("PTO_INCRE_FLASH_ATTENTION_PV_BASE_N", str(min(variant.head_dim, 64))),
        "PTO_INCRE_FLASH_ATTENTION_PV_BASE_K": os.environ.get("PTO_INCRE_FLASH_ATTENTION_PV_BASE_K", "16"),
        "PTO_INCRE_FLASH_ATTENTION_PV_BLOCK_DIM": os.environ.get("PTO_INCRE_FLASH_ATTENTION_PV_BLOCK_DIM", "8"),
        "PTO_INCRE_FLASH_ATTENTION_SOFTMAX_BLOCK_DIM": softmax_block_dim,
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                for _ in range(spec.bench.warmup):
                    run_pto_incre_flash_attention_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_incre_flash_attention_variant(wrapper, inputs)
                    torch.npu.synchronize()
        ...
        report_path = Path(artifacts_dir) / "ptodsl_incre_flash_attention_benchmark.json"
