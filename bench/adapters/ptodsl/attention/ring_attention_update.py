...
from pto_kernels.ops.attention.ring_attention_update.runtime import (
    VARIANTS,
    make_ring_attention_update_inputs,
    run_pto_ring_attention_update_variant,
)


KERNEL = "python/pto_kernels/ops/attention/ring_attention_update/kernel.py"
META = "python/pto_kernels/ops/attention/ring_attention_update/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_RING_ATTENTION_UPDATE_ROWS": str(variant.rows),
        "PTO_RING_ATTENTION_UPDATE_HEAD_DIM": str(variant.head_dim),
        "PTO_RING_ATTENTION_UPDATE_BLOCK_DIM": os.environ.get("PTO_RING_ATTENTION_UPDATE_BLOCK_DIM", "8"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                inputs = make_ring_attention_update_inputs(variant, device_index=int(spec.device.get("id", 0)))

                for _ in range(spec.bench.warmup):
                    run_pto_ring_attention_update_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_ring_attention_update_variant(wrapper, inputs)
                    torch.npu.synchronize()
                ...
                attn_diff = (output["attn_out"].cpu() - ref["attn_out"]).abs().max().item()
                max_diff = (output["softmax_max_out"].cpu() - ref["softmax_max_out"]).abs().max().item()
                sum_diff = (output["softmax_sum_out"].cpu() - ref["softmax_sum_out"]).abs().max().item()
                worst = max(attn_diff, max_diff, sum_diff)
                variant_reports.append(
                    {
                        ...
                        "correctness": {
                            ...
                            "softmax_max_out_max_abs_diff": max_diff,
                            "softmax_sum_out_max_abs_diff": sum_diff,
                            "max_abs_diff": worst,
                            "passes": bool(worst <= spec.correctness.atol),
                        },
                    }
                )
        ...
        report_path = Path(artifacts_dir) / "ptodsl_ring_attention_update_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "tnd_sp2_merge_lastdim8",
