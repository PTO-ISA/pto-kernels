...
from pto_kernels.ops.gmm.grouped_matmul_add.runtime import (
    VARIANT,
    VARIANTS,
    make_dense_grouped_matmul_add_inputs,
    run_pto_variant,
)


KERNEL = "python/pto_kernels/ops/gmm/grouped_matmul_add/kernel.py"
META = "python/pto_kernels/ops/gmm/grouped_matmul_add/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_GROUPED_MATMUL_ADD_M": str(variant.m),
        "PTO_GROUPED_MATMUL_ADD_K": str(variant.k),
        "PTO_GROUPED_MATMUL_ADD_N": str(variant.n),
        "PTO_GROUPED_MATMUL_ADD_BASE_M": os.environ.get("PTO_GROUPED_MATMUL_ADD_BASE_M", "16"),
        "PTO_GROUPED_MATMUL_ADD_BASE_N": os.environ.get("PTO_GROUPED_MATMUL_ADD_BASE_N", "64"),
        "PTO_GROUPED_MATMUL_ADD_BASE_K": os.environ.get("PTO_GROUPED_MATMUL_ADD_BASE_K", "64"),
        "PTO_GROUPED_MATMUL_ADD_BLOCK_DIM": os.environ.get("PTO_GROUPED_MATMUL_ADD_BLOCK_DIM", "16"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            ...
            spec_obj = importlib.util.spec_from_file_location(f"pto_grouped_matmul_add_kernel_{variant.label}", kernel_file)
            ...
            with temporary_env(_variant_env(variant)):
                ...
                inputs = make_dense_grouped_matmul_add_inputs(variant, device_index=int(spec.device.get("id", 0)))
        ...
        report_path = Path(artifacts_dir) / "ptodsl_grouped_matmul_add_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "bf16_transposex_matmul_add_f32",
    }
    report_path = Path(artifacts_dir) / "ptodsl_grouped_matmul_add_benchmark.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    report["report_path"] = str(report_path)
