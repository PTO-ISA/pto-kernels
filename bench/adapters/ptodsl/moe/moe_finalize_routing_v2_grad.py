...
from pto_kernels.ops.moe.moe_finalize_routing_v2_grad.runtime import (
    VARIANTS,
    make_top1_finalize_routing_v2_grad_inputs,
    run_pto_moe_finalize_routing_v2_grad_variant,
)
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_MOE_FINALIZE_V2_GRAD_TOKENS": str(variant.tokens),
        "PTO_MOE_FINALIZE_V2_GRAD_HIDDEN": str(variant.hidden_size),
        "PTO_MOE_FINALIZE_V2_GRAD_EXPERTS": str(variant.experts),
        "PTO_MOE_FINALIZE_V2_GRAD_BLOCK_DIM": os.environ.get("PTO_MOE_FINALIZE_V2_GRAD_BLOCK_DIM", "8"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                inputs = make_top1_finalize_routing_v2_grad_inputs(variant, device_index=int(spec.device.get("id", 0)))

                for _ in range(spec.bench.warmup):
                    run_pto_moe_finalize_routing_v2_grad_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_finalize_routing_v2_grad_variant(wrapper, inputs)
                    torch.npu.synchronize()
                ...
                grad_expanded_x_out, grad_scales_out = output
                grad_expanded_x_diff = (
                    grad_expanded_x_out.float().cpu() - inputs["reference_grad_expanded_x_out"]
                ).abs().max().item()
                grad_scales_diff = (
                    grad_scales_out.float().cpu() - inputs["reference_grad_scales_out"]
                ).abs().max().item()
                variant_reports.append(
                    {
                        ...
                        "correctness": {
                            "grad_expanded_x_max_abs_diff": grad_expanded_x_diff,
                            "grad_scales_max_abs_diff": grad_scales_diff,
                            "max_abs_diff": max(grad_expanded_x_diff, grad_scales_diff),
                        },
                    }
                )
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_finalize_routing_v2_grad_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "top1_finalize_routing_v2_grad_with_scale_and_bias",
        "variant_reports": variant_reports,
        "artifact_paths": artifact_paths,
        "block_utilization": {
            "nominal_tokens": VARIANTS[1].tokens,
            "requested_block_dim": int(os.environ.get("PTO_MOE_FINALIZE_V2_GRAD_BLOCK_DIM", "8")),
