...
from pto_kernels.ops.moe.moe_finalize_routing_v2.runtime import (
    VARIANTS,
    make_top1_finalize_routing_v2_inputs,
    run_pto_moe_finalize_routing_v2_variant,
)


KERNEL = "python/pto_kernels/ops/moe/moe_finalize_routing_v2/kernel.py"
META = "python/pto_kernels/ops/moe/moe_finalize_routing_v2/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_MOE_FINALIZE_V2_TOKENS": str(variant.tokens),
        "PTO_MOE_FINALIZE_V2_HIDDEN": str(variant.hidden_size),
        "PTO_MOE_FINALIZE_V2_EXPERTS": str(variant.experts),
        "PTO_MOE_FINALIZE_V2_BLOCK_DIM": os.environ.get("PTO_MOE_FINALIZE_V2_BLOCK_DIM", "8"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                inputs = make_top1_finalize_routing_v2_inputs(variant, device_index=int(spec.device.get("id", 0)))

                for _ in range(spec.bench.warmup):
                    run_pto_moe_finalize_routing_v2_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_finalize_routing_v2_variant(wrapper, inputs)
                    torch.npu.synchronize()
                ...
                token_diff = (output.float().cpu() - inputs["reference"]).abs().max().item()
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_finalize_routing_v2_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "top1_finalize_routing_v2_x1_x2_bias_scales",
        "variant_reports": variant_reports,
        "artifact_paths": artifact_paths,
        "block_utilization": {
            "nominal_tokens": VARIANTS[1].tokens,
            "requested_block_dim": int(os.environ.get("PTO_MOE_FINALIZE_V2_BLOCK_DIM", "8")),
