...
from pto_kernels.ops.moe.moe_token_unpermute_with_routing_map.runtime import (
    ...
    run_pto_moe_token_unpermute_with_routing_map_variant,
)


KERNEL = "python/pto_kernels/ops/moe/moe_token_unpermute_with_routing_map/kernel.py"
META = "python/pto_kernels/ops/moe/moe_token_unpermute_with_routing_map/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_MOE_UNPERMUTE_ROUTING_MAP_TOKENS": str(variant.tokens),
        "PTO_MOE_UNPERMUTE_ROUTING_MAP_HIDDEN": str(variant.hidden_size),
        "PTO_MOE_UNPERMUTE_ROUTING_MAP_BLOCK_DIM": os.environ.get("PTO_MOE_UNPERMUTE_ROUTING_MAP_BLOCK_DIM", "20"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                for _ in range(spec.bench.warmup):
                    run_pto_moe_token_unpermute_with_routing_map_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_token_unpermute_with_routing_map_variant(wrapper, inputs)
                    torch.npu.synchronize()
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_token_unpermute_with_routing_map_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "top1_host_routing_map_unpermute",
