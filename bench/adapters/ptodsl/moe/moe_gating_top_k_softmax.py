...
from pto_kernels.ops.moe.moe_gating_top_k_softmax.runtime import (
    VARIANTS,
    make_moe_gating_top_k_softmax_inputs,
    run_pto_moe_gating_top_k_softmax_variant,
)


KERNEL = "python/pto_kernels/ops/moe/moe_gating_top_k_softmax/kernel.py"
META = "python/pto_kernels/ops/moe/moe_gating_top_k_softmax/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_MOE_GATING_TOPK_SOFTMAX_ROWS": str(variant.rows),
        "PTO_MOE_GATING_TOPK_SOFTMAX_EXPERTS": str(variant.experts),
        "PTO_MOE_GATING_TOPK_SOFTMAX_BLOCK_DIM": os.environ.get("PTO_MOE_GATING_TOPK_SOFTMAX_BLOCK_DIM", "8"),
    }
    ...
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                inputs = make_moe_gating_top_k_softmax_inputs(variant, device_index=int(spec.device.get("id", 0)))
                probs_tmp = torch.empty((variant.rows, variant.experts), dtype=torch.float16).npu()

                for _ in range(spec.bench.warmup):
                    run_pto_moe_gating_top_k_softmax_variant(wrapper, {**inputs, "probs_tmp": probs_tmp})
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_gating_top_k_softmax_variant(wrapper, {**inputs, "probs_tmp": probs_tmp})
                    torch.npu.synchronize()
                ...
                y_out, expert_idx_out, row_idx_out = output
                ...
                row_diff = (row_idx_out.to(torch.int32).cpu() - inputs["reference_row_idx_out"]).abs().max().item()
                variant_reports.append(
                    {
                        ...
                        "correctness": {
                            ...
                            "row_idx_max_abs_diff": float(row_diff),
                            "max_abs_diff": max(y_diff, float(expert_diff), float(row_diff)),
                        },
                    }
                )
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_gating_top_k_softmax_benchmark.json"
        ...
        "correctness": {
            ...
            "passes": bool(
                all(
                    ...
                    and item["correctness"]["row_idx_max_abs_diff"] == 0
                    for item in variant_reports
                )
            ),
        },
        "reference_contract": "top1_softmax_no_finished_2d",
