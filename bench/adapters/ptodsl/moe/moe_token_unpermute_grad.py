...
from pto_kernels.ops.moe.moe_token_unpermute_grad.runtime import (
    ...
    run_pto_moe_token_unpermute_grad_variant,
)


KERNEL = "python/pto_kernels/ops/moe/moe_token_unpermute_grad/kernel.py"
META = "python/pto_kernels/ops/moe/moe_token_unpermute_grad/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_MOE_UNPERMUTE_GRAD_TOKENS": str(variant.tokens),
        "PTO_MOE_UNPERMUTE_GRAD_HIDDEN": str(variant.hidden_size),
        "PTO_MOE_UNPERMUTE_GRAD_BLOCK_DIM": os.environ.get("PTO_MOE_UNPERMUTE_GRAD_BLOCK_DIM", "20"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                for _ in range(spec.bench.warmup):
                    run_pto_moe_token_unpermute_grad_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_token_unpermute_grad_variant(wrapper, inputs)
                    torch.npu.synchronize()
                ...
                variant_reports.append(
                    {
                        ...
                        "correctness": {
                            ...
                            "max_abs_diff": max(token_diff, probs_diff),
                        },
                    }
                )
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_token_unpermute_grad_benchmark.json"
