...
from pto_kernels.ops.moe.moe_gating_top_k_softmax_v2.runtime import (
    VARIANTS,
    make_moe_gating_top_k_softmax_v2_inputs,
    run_pto_moe_gating_top_k_softmax_v2_variant,
)


KERNEL = "python/pto_kernels/ops/moe/moe_gating_top_k_softmax_v2/kernel.py"
META = "python/pto_kernels/ops/moe/moe_gating_top_k_softmax_v2/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_MOE_GATING_TOPK_SOFTMAX_V2_ROWS": str(variant.rows),
        "PTO_MOE_GATING_TOPK_SOFTMAX_V2_EXPERTS": str(variant.experts),
        "PTO_MOE_GATING_TOPK_SOFTMAX_V2_BLOCK_DIM": os.environ.get("PTO_MOE_GATING_TOPK_SOFTMAX_V2_BLOCK_DIM", "8"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                inputs = make_moe_gating_top_k_softmax_v2_inputs(variant, device_index=int(spec.device.get("id", 0)))
                probs_tmp = torch.empty((variant.rows, variant.experts), dtype=torch.float16).npu()

                for _ in range(spec.bench.warmup):
                    run_pto_moe_gating_top_k_softmax_v2_variant(wrapper, {**inputs, "probs_tmp": probs_tmp})
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_gating_top_k_softmax_v2_variant(wrapper, {**inputs, "probs_tmp": probs_tmp})
                    torch.npu.synchronize()
                ...
                variant_reports.append(
                    {
                        ...
                        "correctness": {
                            ...
                            "max_abs_diff": max(y_diff, float(expert_diff)),
                        },
                    }
                )
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_gating_top_k_softmax_v2_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "top1_softmax_v2_renorm0_no_finished_no_softmax_output_2d",
        "variant_reports": variant_reports,
        "artifact_paths": artifact_paths,
        "block_utilization": {
            "nominal_rows": VARIANTS[1].rows,
            "requested_block_dim": int(os.environ.get("PTO_MOE_GATING_TOPK_SOFTMAX_V2_BLOCK_DIM", "8")),
