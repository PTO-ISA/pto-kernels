...
from pto_kernels.ops.moe.moe_init_routing_v2.runtime import (
    ...
    run_pto_moe_init_routing_v2_variant,
)


KERNEL = "python/pto_kernels/ops/moe/moe_init_routing_v2/kernel.py"
META = "python/pto_kernels/ops/moe/moe_init_routing_v2/meta.py"
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                for _ in range(spec.bench.warmup):
                    run_pto_moe_init_routing_v2_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_init_routing_v2_variant(wrapper, inputs)
                    torch.npu.synchronize()
                ...
                cumsum_diff = (
                    expert_tokens_count_or_cumsum - inputs["reference_expert_tokens_count_or_cumsum"]
                ).abs().max().item()
                before_capacity_diff = (
                    expert_tokens_before_capacity - inputs["reference_expert_tokens_before_capacity"]
                ).abs().max().item()
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_init_routing_v2_benchmark.json"
