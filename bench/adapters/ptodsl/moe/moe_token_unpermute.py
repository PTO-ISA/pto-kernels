...
from pto_kernels.ops.moe.moe_token_unpermute.runtime import (
    VARIANT,
    VARIANTS,
    make_top1_unpermute_inputs,
    run_pto_moe_token_unpermute_variant,
)


KERNEL = "python/pto_kernels/ops/moe/moe_token_unpermute/kernel.py"
META = "python/pto_kernels/ops/moe/moe_token_unpermute/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_MOE_UNPERMUTE_TOKENS": str(variant.tokens),
        "PTO_MOE_UNPERMUTE_HIDDEN": str(variant.hidden_size),
        "PTO_MOE_UNPERMUTE_BLOCK_DIM": os.environ.get("PTO_MOE_UNPERMUTE_BLOCK_DIM", "20"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                inputs = make_top1_unpermute_inputs(variant, device_index=int(spec.device.get("id", 0)))

                for _ in range(spec.bench.warmup):
                    run_pto_moe_token_unpermute_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_token_unpermute_variant(wrapper, inputs)
                    torch.npu.synchronize()
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_token_unpermute_benchmark.json"
    ...
    report = {
        ...
        "reference_contract": "top1_host_inverse_gather_map_unpermute",
