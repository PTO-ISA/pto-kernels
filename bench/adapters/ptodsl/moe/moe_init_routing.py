...
from pto_kernels.ops.moe.moe_init_routing.runtime import (
    ...
    run_pto_moe_init_routing_variant,
)


KERNEL = "python/pto_kernels/ops/moe/moe_init_routing/kernel.py"
META = "python/pto_kernels/ops/moe/moe_init_routing/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_MOE_INIT_ROUTING_TOKENS": str(variant.tokens),
        "PTO_MOE_INIT_ROUTING_HIDDEN": str(variant.hidden_size),
        "PTO_MOE_INIT_ROUTING_BLOCK_DIM": os.environ.get("PTO_MOE_INIT_ROUTING_BLOCK_DIM", "8"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                for _ in range(spec.bench.warmup):
                    run_pto_moe_init_routing_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_init_routing_variant(wrapper, inputs)
                    torch.npu.synchronize()
                ...
                x_diff = (expanded_x - inputs["reference_expanded_x"]).abs().max().item()
                row_diff = (expanded_row_idx - inputs["reference_expanded_row_idx"]).abs().max().item()
                expert_diff = (expanded_expert_idx - inputs["reference_expanded_expert_idx"]).abs().max().item()
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_init_routing_benchmark.json"
