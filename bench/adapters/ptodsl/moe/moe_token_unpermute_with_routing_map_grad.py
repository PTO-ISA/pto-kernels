...
from pto_kernels.ops.moe.moe_token_unpermute_with_routing_map_grad.runtime import (
    VARIANTS,
    make_routing_map_unpermute_grad_inputs,
    run_pto_moe_token_unpermute_with_routing_map_grad_variant,
)


KERNEL = "python/pto_kernels/ops/moe/moe_token_unpermute_with_routing_map_grad/kernel.py"
META = "python/pto_kernels/ops/moe/moe_token_unpermute_with_routing_map_grad/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_MOE_UNPERMUTE_ROUTING_MAP_GRAD_TOKENS": str(variant.tokens),
        "PTO_MOE_UNPERMUTE_ROUTING_MAP_GRAD_HIDDEN": str(variant.hidden_size),
        "PTO_MOE_UNPERMUTE_ROUTING_MAP_GRAD_BLOCK_DIM": os.environ.get("PTO_MOE_UNPERMUTE_ROUTING_MAP_GRAD_BLOCK_DIM", "20"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                inputs = make_routing_map_unpermute_grad_inputs(variant, device_index=int(spec.device.get("id", 0)))

                for _ in range(spec.bench.warmup):
                    run_pto_moe_token_unpermute_with_routing_map_grad_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_token_unpermute_with_routing_map_grad_variant(wrapper, inputs)
                    torch.npu.synchronize()
                ...
                token_diff = (permuted_grad.float().cpu() - inputs["reference_permuted_tokens_grad"]).abs().max().item()
                probs_diff = (probs_grad.float().cpu() - inputs["reference_probs_grad"]).abs().max().item()
                variant_reports.append(
                    {
                        ...
                        "correctness": {
                            ...
                            "max_abs_diff": token_diff,
                            "supported_outputs": ["permuted_tokens_grad"],
                            "unsupported_outputs": ["probs_grad"],
                        },
                    }
                )
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_token_unpermute_with_routing_map_grad_benchmark.json"
    ...
    report = {
        ...
        "correctness": {
            ...
            "passes": bool(all(item["correctness"]["token_max_abs_diff"] <= spec.correctness.atol for item in variant_reports)),
        },
        "reference_contract": "top1_routing_map_unpermute_grad_no_probs",
