...
from pto_kernels.ops.moe.moe_compute_expert_tokens.runtime import (
    ...
    run_pto_moe_compute_expert_tokens_variant,
)


KERNEL = "python/pto_kernels/ops/moe/moe_compute_expert_tokens/kernel.py"
META = "python/pto_kernels/ops/moe/moe_compute_expert_tokens/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_MOE_COMPUTE_EXPERT_TOKENS_ROWS": str(variant.rows),
        "PTO_MOE_COMPUTE_EXPERT_TOKENS_EXPERTS": str(variant.num_experts),
        "PTO_MOE_COMPUTE_EXPERT_TOKENS_BLOCK_DIM": os.environ.get(
            "PTO_MOE_COMPUTE_EXPERT_TOKENS_BLOCK_DIM", "20"
        ),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                for _ in range(spec.bench.warmup):
                    run_pto_moe_compute_expert_tokens_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_compute_expert_tokens_variant(wrapper, inputs)
                    torch.npu.synchronize()
                ...
                out_diff = (output - inputs["reference_out"]).abs().max().item()
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_compute_expert_tokens_benchmark.json"
