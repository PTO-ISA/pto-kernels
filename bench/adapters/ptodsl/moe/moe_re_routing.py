...
from pto_kernels.ops.moe.moe_re_routing.runtime import (
    ...
    run_pto_moe_re_routing_variant,
)


KERNEL = "python/pto_kernels/ops/moe/moe_re_routing/kernel.py"
META = "python/pto_kernels/ops/moe/moe_re_routing/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        "PTO_MOE_RE_ROUTING_TOKENS": str(variant.tokens),
        "PTO_MOE_RE_ROUTING_HIDDEN": str(variant.hidden_size),
        "PTO_MOE_RE_ROUTING_RANKS": str(variant.ranks),
        "PTO_MOE_RE_ROUTING_EXPERTS": str(variant.experts),
        "PTO_MOE_RE_ROUTING_BLOCK_DIM": os.environ.get("PTO_MOE_RE_ROUTING_BLOCK_DIM", "20"),
    }
...
def benchmark(repo_root, spec, artifacts_dir):
    try:
        ...
        for variant in VARIANTS:
            with temporary_env(_variant_env(variant)):
                ...
                for _ in range(spec.bench.warmup):
                    run_pto_moe_re_routing_variant(wrapper, inputs)
                ...
                for _ in range(spec.bench.repeat):
                    ...
                    output = run_pto_moe_re_routing_variant(wrapper, inputs)
                    torch.npu.synchronize()
                ...
                idx_diff = (
                    permute_idx.to(torch.int32) - inputs["reference_permute_token_idx"].to(torch.int32)
                ).abs().max().item()
                expert_diff = (
                    expert_token_num.to(torch.int32) - inputs["reference_expert_token_num"].to(torch.int32)
                ).abs().max().item()
        ...
        report_path = Path(artifacts_dir) / "ptodsl_moe_re_routing_benchmark.json"
