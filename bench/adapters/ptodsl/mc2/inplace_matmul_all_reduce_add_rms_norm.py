...
KERNEL = "python/pto_kernels/ops/mc2/inplace_matmul_all_reduce_add_rms_norm/kernel.py"
META = "python/pto_kernels/ops/mc2/inplace_matmul_all_reduce_add_rms_norm/meta.py"
...
def _variant_env(variant) -> dict[str, str]:
    return {
        ...
        "PTO_MC2_IMM_ARN_WORLD_SIZE": str(variant.expected_world_size),
        "PTO_MC2_IMM_ARN_M": str(variant.m),
        "PTO_MC2_IMM_ARN_N": str(variant.n),
        "PTO_MC2_IMM_ARN_BLOCK_DIM": os.environ.get("PTO_MC2_IMM_ARN_BLOCK_DIM", "8"),
    }
    ...
    try:
        ...
        with temporary_env(_variant_env(variant)):
            for kernel_path, build_name in (
                ...
                (KERNEL, "build_inplace_add_rms_norm_jit_wrapper"),
            ):
