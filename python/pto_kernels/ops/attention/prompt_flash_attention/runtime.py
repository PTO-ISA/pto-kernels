"""Runtime helpers for the first prompt_flash_attention migration slice."""
...
class PromptFlashAttentionVariant:
    q_seq: int
    kv_seq: int
    head_dim: int
    seed: int
...
VARIANT = PromptFlashAttentionVariant(q_seq=16, kv_seq=16, head_dim=16, seed=0)
VARIANTS = (
    PromptFlashAttentionVariant(q_seq=16, kv_seq=16, head_dim=16, seed=0),
    PromptFlashAttentionVariant(q_seq=64, kv_seq=128, head_dim=64, seed=1),
    PromptFlashAttentionVariant(q_seq=32, kv_seq=128, head_dim=128, seed=2),
)
...
def make_prompt_flash_attention_inputs(
    variant: PromptFlashAttentionVariant = VARIANT,
    *,
    device_index: int = 0,
) -> dict[str, object]:
...
def run_torch_npu_prompt_flash_attention(inputs: dict[str, object]):
...
def run_pto_prompt_flash_attention_variant(wrapper, inputs: dict[str, object]) -> torch.Tensor:
    wrapper(
        inputs["output_pto"],
        inputs["scores_pto"],
        inputs["query_pto"][0, 0].contiguous(),
        inputs["key_t"][0, 0].contiguous(),
