...
def _load_constants(
    mhc_scale_ptr, mhc_base_ptr, mhc_mult3: int, scale0, scale1, scale2, base
):
    c0 = const(0)
...
def build_pre_split_mixes_fwd(
    ...
    def tilekernels_mhc_pre_split_mixes_fwd_kernel(
        ...
        with pto.vector_section():
            ...
            for row_idx in pto.range(row_start, row_end, c1):
                ...
                input_post = tile.subset(
                    input_row, [c0, const(mhc_mult)], [1, mhc_mult]
                )
                ...
                input_comb = tile.subset(
                    input_row, [c0, const(mhc_mult * 2)], [1, mhc_mult2]
                )
    ...
    return to_ir_module(meta_data=meta_data)(tilekernels_mhc_pre_split_mixes_fwd_kernel)
...
def build_pre_split_mixes_bwd(
    ...
    def tilekernels_mhc_pre_split_mixes_bwd_kernel(
        ...
        with pto.vector_section():
            ...
            _muls(
                tile.subset(base, [c0, const(mhc_mult)], [1, mhc_mult]), 0.0, post_acc
            )
            _muls(
                tile.subset(base, [c0, const(mhc_mult * 2)], [1, mhc_mult2]),
                0.0,
                comb_acc,
            )
            ...
            for row_idx in pto.range(c0, num_tokens, c1):
                ...
                pto.store(
                    scaled16, _slice_row16(input_grad_t, row_idx, const(mhc_mult * 2))
                )
    ...
    return to_ir_module(meta_data=meta_data)(tilekernels_mhc_pre_split_mixes_bwd_kernel)
