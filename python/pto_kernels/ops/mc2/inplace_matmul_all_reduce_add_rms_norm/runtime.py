"""Runtime helpers for the phase-2 inplace_matmul_all_reduce_add_rms_norm seed.

The current validated PTO slice models the inplace residual contract through a
separate epilogue output plus host copy-back. Native same-buffer aliasing is
tracked as a remaining PTODSL/PTOAS issue for later closure.
"""
...
class InplaceMatmulAllReduceAddRmsNormVariant:
    m: int = 128
    k: int = 256
    n: int = 128
    dtype: str = "float16"
...
def _resolve_variant(
    variant: InplaceMatmulAllReduceAddRmsNormVariant | None = None,
...
def _reference_outputs(
    ...
    residual_in = _make_rank_tensors(rank, resolved)[2]
    ...
    return residual_y.to(torch.float16), norm_out
...
def baseline_blocker(*, device_index: int) -> dict[str, object]:
    ...
    if detected_npus < VARIANT.expected_world_size:
        return {
            "status": "blocked",
            "reason": (
                f"Need {VARIANT.expected_world_size} NPUs for inplace_matmul_all_reduce_add_rms_norm bring-up, "
    ...
    return {
        "status": "ready",
        "environment": environment,
        "entrypoint": "torch_npu.npu_mm_all_reduce_base + torch_npu.npu_add_rms_norm (inplace residual contract emulated)",
    }
...
def _baseline_worker(
    *,
    ...
    del output_dir
    variant = InplaceMatmulAllReduceAddRmsNormVariant(**variant_dict)
    ...
    ref_residual_y, ref_norm = _reference_outputs(rank, world_size, variant)
...
def run_distributed_baseline_benchmark(
    *,
    variant: InplaceMatmulAllReduceAddRmsNormVariant = VARIANT,
    artifacts_dir: Path,
    warmup: int,
    repeat: int,
) -> dict[str, object]:
    ...
    if launch["status"] != "ok":
        return {
            ...
            "reason": launch.get(
                "reason",
                "Distributed inplace_matmul_all_reduce_add_rms_norm baseline failed.",
            ),
            "variant": variant.as_dict(),
            "world_size": world_size,
...
def _pto_worker(
    *,
    ...
    os.environ["PTO_MC2_IMM_ARN_WORLD_SIZE"] = str(world_size)
    os.environ["PTO_MC2_IMM_ARN_M"] = str(variant.m)
    os.environ["PTO_MC2_IMM_ARN_N"] = str(variant.n)

    mm_module = _load_module_from_path(
        Path(__file__).resolve().parent.parent / "matmul_all_reduce" / "kernel.py",
        "pto_mc2_matmul_all_reduce_kernel_for_iarn",
    )
    ...
    def _run_once() -> tuple[torch.Tensor, torch.Tensor]:
        ...
        arn_wrapper(
            ...
            mm_out,
            residual_seed,
            gamma.view(1, variant.n),
            inv_n,
            eps_row,
        )
        residual_inout.copy_(residual_y)
        return residual_inout, norm_out

    for _ in range(warmup):
        residual_inout.copy_(residual_seed)
        dist.barrier()
        _run_once()
    torch.npu.synchronize()
    dist.barrier()
    ...
    for _ in range(repeat):
        ...
        torch.npu.synchronize()
        start = time.perf_counter()
        pto_residual_y, pto_norm = _run_once()
        torch.npu.synchronize()
        dist.barrier()
        timings_ms.append((time.perf_counter() - start) * 1000.0)
    ...
    *,
    ...
    if launch["status"] != "ok":
        return {
            ...
            "reason": launch.get(
                "reason",
                "Distributed PTO inplace_matmul_all_reduce_add_rms_norm launch failed.",
            ),
            "variant": variant.as_dict(),
            "world_size": world_size,
            "rank_reports": launch.get("rank_reports", []),
