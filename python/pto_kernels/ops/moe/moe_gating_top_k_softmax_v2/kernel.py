"""Constrained PTO-DSL seed for moe_gating_top_k_softmax_v2 on 910B."""
...
class MoeGatingTopKSoftmaxV2Config:
    rows: int
    experts: int
    block_dim: int


def _config() -> MoeGatingTopKSoftmaxV2Config:
    return MoeGatingTopKSoftmaxV2Config(
        rows=tuned_int("PTO_MOE_GATING_TOPK_SOFTMAX_V2_ROWS", 8, valid_values=(8, 128, 256)),
        experts=tuned_int("PTO_MOE_GATING_TOPK_SOFTMAX_V2_EXPERTS", 16, valid_values=(16, 64, 128)),
        block_dim=tuned_int("PTO_MOE_GATING_TOPK_SOFTMAX_V2_BLOCK_DIM", 8, valid_values=(1, 2, 4, 8, 16, 20)),
    )


def _softmax_meta_data(config: MoeGatingTopKSoftmaxV2Config):
    dtype = pto.float16
...
def _select_meta_data(config: MoeGatingTopKSoftmaxV2Config):
    dtype = pto.float16
    i32 = pto.int32
    return {
        "ptr": pto.PtrType(dtype),
...
def _build_softmax_stage(*, config: MoeGatingTopKSoftmaxV2Config, output_dir):
    @jit(
    ...
    def moe_gating_top_k_softmax_v2_stage(probs_ptr: "ptr", x_ptr: "ptr") -> None:
        c0 = const(0)
        c1 = const(1)
    ...
    return moe_gating_top_k_softmax_v2_stage


def _build_select_stage(*, config: MoeGatingTopKSoftmaxV2Config, output_dir):
    @jit(
    ...
    def moe_gating_top_k_softmax_v2_select_stage(
        y_ptr: "ptr",
    ...
    return moe_gating_top_k_softmax_v2_select_stage


class MoeGatingTopKSoftmaxV2Wrapper:
...
def build_jit_wrapper(*, output_dir):
    return MoeGatingTopKSoftmaxV2Wrapper(output_dir=output_dir)
