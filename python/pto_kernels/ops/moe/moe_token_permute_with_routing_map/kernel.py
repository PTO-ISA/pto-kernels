"""Constrained PTO-DSL seed for moe_token_permute_with_routing_map on 910B."""
...
class MoeTokenPermuteWithRoutingMapConfig:
    tokens: int
    hidden: int
    block_dim: int
...
def _config() -> MoeTokenPermuteWithRoutingMapConfig:
    return MoeTokenPermuteWithRoutingMapConfig(
        tokens=tuned_int("PTO_MOE_TOKENS", 8, valid_values=(8, 128, 256)),
        hidden=tuned_int("PTO_MOE_HIDDEN", 16, valid_values=(16, 64, 128)),
        block_dim=tuned_int("PTO_MOE_BLOCK_DIM", 8, valid_values=(1, 2, 4, 8, 16, 20)),
    )


def _permute_meta_data(config: MoeTokenPermuteWithRoutingMapConfig):
    dtype = pto.float16
    i32 = pto.int32
...
def _copy_indices_meta_data(config: MoeTokenPermuteWithRoutingMapConfig):
    i32 = pto.int32
    ptr_i32 = pto.PtrType(i32)
    tensor_i32 = pto.TensorType(rank=1, dtype=i32)
...
def _build_permute_kernel(*, config: MoeTokenPermuteWithRoutingMapConfig, output_dir):
    @jit(
    ...
    def moe_token_permute_with_routing_map_seed(out_ptr: "ptr", tokens_ptr: "ptr", gather_ptr: "ptr_i32") -> None:
        c0 = const(0)
        c1 = const(1)
    ...
    return moe_token_permute_with_routing_map_seed


def _build_copy_indices_kernel(*, config: MoeTokenPermuteWithRoutingMapConfig, output_dir):
    @jit(
    ...
    def moe_token_permute_with_routing_map_copy_indices(out_ptr: "ptr_i32", indices_ptr: "ptr_i32") -> None:
        c0 = const(0)
        c1 = const(1)
        cTokens = const(config.tokens)
    ...
    return moe_token_permute_with_routing_map_copy_indices


class MoeTokenPermuteWithRoutingMapWrapper:
    ...
    def __call__(self, out_ptr, sorted_indices_out_ptr, tokens_ptr, gather_ptr, sorted_indices_in_ptr, stream_ptr=None):
        self._permute(out_ptr, tokens_ptr, gather_ptr, stream_ptr=stream_ptr)
        self._copy(sorted_indices_out_ptr, sorted_indices_in_ptr, stream_ptr=stream_ptr)


def build_jit_wrapper(*, output_dir):
    return MoeTokenPermuteWithRoutingMapWrapper(output_dir=output_dir)
