"""Constrained PTO-DSL seed for moe_token_unpermute_grad on 910B."""
...
class MoeTokenUnpermuteGradConfig:
    tokens: int
    hidden: int
    block_dim: int


def _config() -> MoeTokenUnpermuteGradConfig:
    return MoeTokenUnpermuteGradConfig(
        tokens=tuned_int("PTO_MOE_UNPERMUTE_GRAD_TOKENS", 8, valid_values=(8, 128, 256)),
        hidden=tuned_int("PTO_MOE_UNPERMUTE_GRAD_HIDDEN", 16, valid_values=(16, 64, 128)),
        block_dim=tuned_int("PTO_MOE_UNPERMUTE_GRAD_BLOCK_DIM", 20, valid_values=(1, 2, 4, 8, 16, 20)),
    )


def _meta_data(config: MoeTokenUnpermuteGradConfig):
    dtype = pto.float16
    i32 = pto.int32
...
def _build_unpermute_grad_kernel(*, config: MoeTokenUnpermuteGradConfig, output_dir):
    @jit(
    ...
    def moe_token_unpermute_grad_seed(
    ...
    return moe_token_unpermute_grad_seed


class MoeTokenUnpermuteGradWrapper:
    ...
    def __call__(self, permuted_grad_ptr, probs_grad_ptr, unpermuted_grad_ptr, sorted_indices_ptr, stream_ptr=None):
        self._kernel(
            permuted_grad_ptr,
            probs_grad_ptr,
            unpermuted_grad_ptr,
...
def build_jit_wrapper(*, output_dir):
    return MoeTokenUnpermuteGradWrapper(output_dir=output_dir)
