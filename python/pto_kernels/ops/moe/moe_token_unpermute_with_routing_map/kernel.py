"""Constrained PTO-DSL seed for moe_token_unpermute_with_routing_map on 910B."""
...
class MoeTokenUnpermuteWithRoutingMapConfig:
    tokens: int
    hidden: int
    block_dim: int
...
def _config() -> MoeTokenUnpermuteWithRoutingMapConfig:
    return MoeTokenUnpermuteWithRoutingMapConfig(
        tokens=tuned_int("PTO_MOE_UNPERMUTE_ROUTING_MAP_TOKENS", 8, valid_values=(8, 128, 256)),
        hidden=tuned_int("PTO_MOE_UNPERMUTE_ROUTING_MAP_HIDDEN", 16, valid_values=(16, 64, 128)),
        block_dim=tuned_int("PTO_MOE_UNPERMUTE_ROUTING_MAP_BLOCK_DIM", 20, valid_values=(1, 2, 4, 8, 16, 20)),
    )


def _meta_data(config: MoeTokenUnpermuteWithRoutingMapConfig):
    dtype = pto.float16
    i32 = pto.int32
...
def _build_unpermute_kernel(*, config: MoeTokenUnpermuteWithRoutingMapConfig, output_dir):
    @jit(
    ...
    def moe_token_unpermute_with_routing_map_seed(out_ptr: "ptr", permuted_ptr: "ptr", gather_ptr: "ptr_i32") -> None:
        c0 = const(0)
        c1 = const(1)
    ...
    return moe_token_unpermute_with_routing_map_seed


class MoeTokenUnpermuteWithRoutingMapWrapper:
...
def build_jit_wrapper(*, output_dir):
    return MoeTokenUnpermuteWithRoutingMapWrapper(output_dir=output_dir)
