...
def _assert_clean_merge(sort_cols: int) -> None:
    ...
    while block * 4 <= sort_cols:
        block *= 4
    if block != sort_cols:
        raise ValueError(f"sort_cols={sort_cols} is not a clean power-of-4 merge width")
...
def build_topk_gate(num_experts: int, num_topk: int):
    ...
    return to_ir_module(meta_data=meta_data)(tilekernels_moe_topk_gate_kernel)
...
def build_topk_sum_and_topk_group_idx(
    ...
    def tilekernels_moe_topk_sum_and_topk_group_idx_kernel(
        ...
        with pto.vector_section():
            ...
            tb_group_score_gather_win_u = pto.alloc_tile(
                tile_group_score_gather_win_u32
            )
            ...
            tile.mov(
                tile.subset(tb_inidx_max, [c0, c0], [1, group_sort_ncols]),
                tb_expert_inidx,
            )
            tile.mov(
                tile.subset(tb_inidx_max, [c0, c0], [1, group_score_sort_ncols]),
                tb_group_inidx,
            )
                ...
                with pto.if_context(rows_this_core > c0):
                    for i in pto.range(c0, rows_this_core, c1):
                        ...
                        for group in range(num_groups):
                            ...
                            tile.sort32(
                                tb_group_src_padded, tb_group_sort, tb_expert_inidx
                            )

                            cur_block = _SORT_BLOCK_LEN * _DST_STRIDE
                            while cur_block * 4 <= group_sort_cols:
                                tile.mrgsort(
                                    tb_group_sort, tb_group_sort_tmp, const(cur_block)
                                )
