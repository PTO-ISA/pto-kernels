"""Runtime helpers for the first moe_finalize_routing migration slice."""
...
class MoeFinalizeRoutingVariant:
    tokens: int
    hidden_size: int
    experts: int = 4
    top_k: int = 1
...
VARIANT = MoeFinalizeRoutingVariant(tokens=16, hidden_size=16, seed=0)
VARIANTS = (
    VARIANT,
    MoeFinalizeRoutingVariant(tokens=256, hidden_size=64, seed=1),
    MoeFinalizeRoutingVariant(tokens=128, hidden_size=128, seed=2),
)
...
def make_top1_finalize_routing_inputs(
    variant: MoeFinalizeRoutingVariant = VARIANT,
    *,
    device_index: int = 0,
) -> dict[str, object]:
    ...
    expanded_expert_idx_cpu = torch.randint(
        low=0,
    ...
    row_ids = expanded_row_idx_cpu.view(variant.tokens, variant.top_k)[:, 0].to(torch.int64)
    expert_ids = expanded_expert_idx_cpu[:, 0].to(torch.int64)
    reference = (
        x1_cpu.float()
        + scales_cpu[:, :1].float()
        * (
    ...
    return {
        ...
        "expanded_expert_idx": expanded_expert_idx_cpu.npu(),
        "out_pto": torch.empty_like(x1_cpu).npu(),
        "reference": reference,
    }


def run_torch_npu_moe_finalize_routing(inputs: dict[str, object]):
    return torch_npu.npu_moe_finalize_routing(
        ...
        inputs["expanded_expert_idx"],
        inputs["variant"]["drop_pad_mode"],
    )


def run_pto_moe_finalize_routing_variant(wrapper, inputs: dict[str, object]):
    wrapper(
        inputs["out_pto"],
        inputs["expanded_x"],
        inputs["x1"],
        ...
        inputs["scales"],
        inputs["expanded_row_idx"],
        inputs["expanded_expert_idx"].reshape(-1),
    )
