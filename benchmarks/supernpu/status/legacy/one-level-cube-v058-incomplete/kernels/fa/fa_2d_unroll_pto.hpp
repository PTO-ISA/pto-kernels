...
void flash_attention_2d_unroll_pto(dtype* out_ptr, dtype* q_ptr, dtype* k_ptr, dtype* v_ptr) {
    ...
    //
    // QK score:
    //   tileW_out: CUBE 输出 tile，logical shape [kTm, kTk]，dtype=float。
    //   tileW: VEC score tile，ColMajor，logical shape [kTm, kTk]。
    ...
    for (int i = 0; i < Qb; i+=Xdim) {
        ...
        for (int j = 0; j < Kb; j+=Ydim) {
            ...
            tileW tW[Xdim][Ydim];
            // QK score:
            //   TMATMUL: tileQ [kTm, qD] x tileK^T [qD, kTk] -> tileW [kTm, kTk] float
            ...
            //   TMATMUL output is consumed directly as VEC float tPV[x] [kTm,vD]
            //
            // After this block:
