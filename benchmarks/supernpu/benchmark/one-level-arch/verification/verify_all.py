...
def compare(a, b, name, rtol=1e-3, atol=1e-4):
    ...
    if close:
        ...
        print(f"  ✓ {name}: PASS (shape={a.shape}, max_err={max_err})")
    else:
        diff = np.abs(a.astype(np.float64) - b.astype(np.float64))
        print(
            f"  ✗ {name}: FAIL (shape={a.shape}, max_err={np.max(diff):.6f}, mean_err={np.mean(diff):.6f})"
        )
    return close
...
def verify_fa_2d_unroll(Sq=256, Skv=512, qD=128, vD=128, dtype=torch.float16):
    ...
    output = attn @ V.float()  # [Sq, vD]
    return {"Q": Q, "K": K, "V": V}, {"O": output.to(dtype).float()}
...
def verify_sfa(Sq=256, Skv=512, qD=128, vD=128, kTm=16, kTk=32, dtype=torch.float16):
    ...
    output = torch.zeros(Sq, vD, dtype=torch.float32)
    for i in range(Qb):
        ...
        p = F.softmax(s, dim=-1)
        output[i * kTm : (i + 1) * kTm] = p @ v_sel

    return {"Q": Q, "K": K, "V": V, "kv_idx": kv_idx, "kv_off": kv_off}, {"O": output}
...
def verify_concat_gather(N_tables=1000, Rows=64, Cols=2, dtype=torch.int32):
    ...
    return {"in": in_flat, "in_shape": in_shape, "out_shape": out_shape}, {
        "out": out_flat
    }
...
def verify_group_count(NumTokens=16, NumTopk=8, NumGroups=32):
    group_idx = randint(
        (NumTokens, NumTopk), low=0, high=NumGroups, dtype=torch.int32, seed=1
    )
...
def verify_aux_fi(NumTokens=16, NumTopk=8, NumExperts=32, num_aux_topk=8):
    topk_idx = randint(
        (NumTokens, NumTopk), low=0, high=NumExperts, dtype=torch.int32, seed=1
    )
...
def verify_per_token_cast(
    M=16, Npc=16, max_val=448.0, clamp_min=1e-6, dtype=torch.bfloat16
):
...
def verify_per_channel_cast(
    Npt=16, Hidden=32, max_val=448.0, clamp_min=1e-6, dtype=torch.bfloat16
):
...
def verify_swiglu(
    M=16, Hidden=16, Npc=16, max_val=448.0, clamp_min=1e-6, dtype=torch.bfloat16
):
...
def verify_reduce_fused(
    NumTokens=16, Hidden=64, NumTopk=4, NumExpanded=32, dtype=torch.bfloat16
):
    x = randn((NumExpanded, Hidden), dtype=dtype, seed=1)
    w = randn((NumTokens, NumTopk), dtype=torch.float32, seed=2).abs()
    pos = randint(
        (NumTokens, NumTopk), low=0, high=NumExpanded, dtype=torch.int32, seed=3
    )
...
def verify_mask_indices_by_tp(
    NumTokens=16, NumTopk=8, per_gpu=1, per_dp=1, num_tp_ranks=1, tp_rank=0
):
...
def verify_engram_hash(NumTokens=16, MaxNgramSize=8, NumEmbedPerNgram=8):
    ngram_ids = randint(
        (NumTokens, MaxNgramSize), low=1, high=1000, dtype=torch.int32, seed=1
    )
        ...
        for n in range(MaxNgramSize):
            ...
            if n > 0:
                for j in range(NumEmbedPerNgram):
                    ...
                    output[t, col] = (hash_val % int(vocab_sizes[col].item())) + int(
                        offsets[col].item()
                    )
    return {
...
def verify_expand_to_fused(
    NumTokens=16, Hidden=64, NumTopk=4, NumExpanded=32, dtype=torch.bfloat16
):
    x = randn((NumTokens, Hidden), dtype=dtype, seed=1)
    token_topk_to_pos = randint(
        (NumTokens, NumTopk), low=-1, high=NumExpanded, dtype=torch.int32, seed=2
    )
    NumExperts_local = 8
    pos_to_expert = randint(
        (NumExpanded,), low=-1, high=NumExperts_local, dtype=torch.int32, seed=3
    )
...
def main():
    ...
    parser.add_argument(
        "--kernel", type=str, default="all", help="Kernel name (or 'all')"
    )
    parser.add_argument(
        "--print", action="store_true", help="Print input/output tensor shapes"
    )
    parser.add_argument(
        "--export", type=str, default=None, help="Export binary files to directory"
    )
